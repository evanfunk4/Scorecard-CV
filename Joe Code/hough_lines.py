...
def extract_grid_mask(img_gray: np.ndarray,
                      h_kernel_ratio: float = 0.15,
                      v_kernel_ratio: float = 0.15) -> tuple[np.ndarray, np.ndarray]:
    
    h, w = img_gray.shape
 
    img_normalized = cv2.adaptiveThreshold(
        img_gray, 255,
    ...
    _, binary = img_normalized, img_normalized  
...
def rotate_image(img: np.ndarray, degrees: int) -> np.ndarray:
    ...
    if degrees == 0:
        return img
    raise ValueError("rotate_image only supports 0, 90, -90, and 180 degrees")
...
def draw_debug(img: np.ndarray,
    ...
    for y in h_lines:
        cv2.line(out, (0, y), (w_img, y), (0, 255, 0), 20)   # green H-lines
    for x in v_lines:
        cv2.line(out, (x, 0), (x, h_img), (0, 0, 255), 20)   # red V-lines
    for cell in cells:
        cv2.rectangle(out,
                      (cell['x'], cell['y']),
                      (cell['x'] + cell['w'], cell['y'] + cell['h']),
                      (255, 0, 0), 20)                         # blue cell boxes
    return out
...
def process_image(img_path: Path,
    ...
    if rotate_degrees is not None and rotate_degrees != 0:
        img_bgr = rotate_image(img_bgr, rotate_degrees)
        output_name = f"{img_path.stem}_rot{abs(rotate_degrees)}{img_path.suffix}"
 
    lab = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2LAB)
    l, a, b = cv2.split(lab)
    clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
    l = clahe.apply(l)
    img_bgr_norm = cv2.cvtColor(cv2.merge([l, a, b]), cv2.COLOR_LAB2BGR)
    img_gray = cv2.cvtColor(img_bgr_norm, cv2.COLOR_BGR2GRAY)
    ...
    if debug_dir is not None:
        ...
        cv2.imwrite(str(debug_dir / output_name), debug_img)
 
    cv2.imwrite(str(images_dir / output_name), img_bgr)
 
    h_img, w_img = img_bgr.shape[:2]
    return {"id": image_id, "file_name": output_name,
...
def run(input_dir: str,
    ...
    for image_id, img_path in enumerate(img_files):
        ...
        record = process_image(img_path, image_id,
                               cells_dir, images_dir, debug_dir,
...
def main() -> None:
    ...
    parser.add_argument("--rotate",   type=int, default=0,
                        help="Rotate input images before processing. Supported: -90, 0, 90, 180. Default is -90 for rotated training output.")
    ...
    rotate_degrees = args.rotate if args.rotate != 0 else None
