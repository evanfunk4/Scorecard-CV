...
class PreprocessConfig:
    ...
    upright_180_margin: float = 2.6
    upright_allow_180_without_osd: bool = False
    deskew: bool = True
...
def _estimate_upright_rotation_quadrants(image_bgr: np.ndarray, cfg: PreprocessConfig) -> int:
    ...
    if (
        ...
        candidate_k = (1, 3)
    elif not cfg.upright_allow_180_without_osd:
        # Without a reliable OSD signal, 180-degree flips are often ambiguous.
        candidate_k = tuple(k for k in candidate_k if k != 2)
    ...
    # Guard against unnecessary 180 flips: many scorecards are near-symmetric.
    # Require a clearer score margin before applying 180 deg rotation.
    if int(best_k) == 2 and 0 in k_scores:
        margin = float(k_scores[2] - k_scores[0])
        if margin < float(cfg.upright_180_margin):
            return 0
    return best_k
